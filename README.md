...
2. Set `frontend.allowed_origin` to the exact frontend origin, for example `https://your-pages-domain.pages.dev`.
3. Before building `frontend/glassx`, set `VITE_BOOTSTRAP_API_BASE` to the plugin HTTP base URL, for example `https://mc.example.com:8080`.
4. Do not append `/api` to `VITE_BOOTSTRAP_API_BASE`; the frontend derives API and WebSocket URLs automatically.
5. Build `frontend/glassx` and deploy the generated `dist/` files to your static host.
...
- Main plugin jar: `plugin/target/verifymc-1.7.5.jar`
- Proxy plugin jar: `plugin-proxy/target/verifymc-proxy-1.7.5.jar`
