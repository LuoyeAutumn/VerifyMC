...
1. 在 `config.yml` 中设置 `auth_methods: [captcha]`（最快启动方式）。其他选项：`[email]`、`[sms]`，或任意组合如 `[email, captcha]`。
...
- 主插件 jar：`plugin/target/verifymc-1.7.7.jar`
- 代理插件 jar：`plugin-proxy/target/verifymc-proxy-1.7.7.jar`
