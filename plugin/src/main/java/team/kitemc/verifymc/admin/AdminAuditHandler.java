...
import team.kitemc.verifymc.audit.AuditRecord;
...
public class AdminAuditHandler implements HttpHandler {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        if (AdminAuthUtil.requireAdmin(exchange, authContext, AdminAction.AUDIT) == null) return;

        AuditQuery query = parseQuery(exchange.getRequestURI().getQuery());
        AuditPage page = auditService.query(query);
        ...
        resp.put("availableActions", new JSONArray(auditService.getAvailableActions()));
