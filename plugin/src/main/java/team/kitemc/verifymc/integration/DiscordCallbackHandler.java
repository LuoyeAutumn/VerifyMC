...
public class DiscordCallbackHandler implements HttpHandler {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        if (code == null || state == null) {
            html = buildCallbackHtml(false, messageResolver.apply("discord.missing_code_state", language));
        } else {
            DiscordService.DiscordCallbackResult result = discordService.handleCallback(code, state);
            boolean success = result.success;
            html = buildCallbackHtml(success, success
                    ? messageResolver.apply("discord.linked_successfully", language)
                    : result.message);
        }
