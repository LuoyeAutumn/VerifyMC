...
public final class CorsSupport {
    ...
    public static void applyCorsHeaders(PlatformServices ctx, HttpExchange exchange, List<String> allowedOrigins) {
        String requestOrigin = exchange.getRequestHeaders().getFirst("Origin");
    ...
    public static boolean handlePreflight(PlatformServices ctx, HttpExchange exchange, List<String> allowedOrigins) throws IOException {
    ...
    private static void logRejectedOrigin(PlatformServices ctx, HttpExchange exchange, String requestOrigin) {
        if (ctx == null) {
            return;
        }
